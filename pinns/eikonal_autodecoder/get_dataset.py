...
def get_eikonal_bcs(charts_path, x, y, charts3d, N=50, seed=37):
    ...
    chart_in_mesh_indices, mesh_in_chart_indices = find_verts_in_charts(charts3d, bcs_points)

    logging.info("gt solution indices: %s", idx_train)

    for chart_key in mesh_in_chart_indices.keys():
        if len(mesh_in_chart_indices[chart_key]) > 0 and len(chart_in_mesh_indices[chart_key]) > 0:

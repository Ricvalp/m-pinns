...
class FilmLayer(nn.Module):
    ...
    def __call__(self, x, condition):
        ...
        film_params = nn.Dense(features=2 * self.features, use_bias=self.use_bias)(
            condition
        )
...
class SirenLayer(nn.Module):
    ...
    def __call__(self, x):
        ...
        x = nn.Dense(
            features=self.features, use_bias=self.use_bias, kernel_init=kernel_init
        )(x)
...
class FilmSiren(nn.Module):
    ...
    def __call__(self, x, condition=None):
        ...
        if self.use_film and condition is None:
            raise ValueError(
                "FiLM conditioning requires a condition tensor, but none was provided"
            )

        # Initial SIREN layer with different initialization
        x = SirenLayer(features=self.features[0], w0=self.w0_initial, is_first=True)(x)
        ...
        for i, feat in enumerate(self.features[1:]):
            x = SirenLayer(features=feat, w0=self.w0)(x)
    ...
    def setup(self):
        ...
        if self.cond_dim is not None:
            self.condition_encoder = ConditionEncoder(
                features=self.cond_encoder_features
            )
            use_film = True
        else:
            use_film = False
...
def test_siren_model():
    ...
    y, x = jnp.meshgrid(
        jnp.linspace(-1, 1, img_size), jnp.linspace(-1, 1, img_size), indexing="ij"
    )

...
def create_sphere_dataset(
    radius=1.0,
    density=2,
    points_per_unit_area=2,
    subset_cardinality=None,
    seed=42,
    center=True,
    output_dir="./datasets/sphere",
):
    """
...
def visualize_geodesic_distances(vertices, faces, radius, density):
    ...
    triang = ax.plot_trisurf(
        vertices[:, 0], vertices[:, 1], vertices[:, 2], triangles=faces, cmap=cmap
    )
...
def visualize_geodesic_contours(radius=1.0, density=3):
    ...
    contour_levels = np.linspace(
        0, max_distance, 13
    )  # 0 to 180 degrees in 15-degree increments
    ...
    unique_handles = [
        handles[labels.index(label)] for label in unique_labels if label in labels
    ]
...
def compare_analytical_vs_mesh_geodesic():
    ...
    for density in densities:
        ...
        mesh_distances = geodesic_distances_from_pole_vectorized(
            closest_vertices, radius
        )
    ...
    for r in results:
        print(
            f"{r['density']:<8} {r['num_vertices']:<10} {r['max_error']:<15.8f} {r['mean_error']:<15.8f}"
        )
...
if __name__ == "__main__":
    ...
    vertices, faces = generate_sphere_mesh(
        radius=radius, density=density, output_file=output_file
    )

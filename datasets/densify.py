...
if __name__ == "__main__":
    ...
    output_obj_file = obj_path + 'dense_' + obj_name # Name for the output denser mesh file
    ...
    # Densify the mesh
    densify_mesh(input_obj_file, output_obj_file, subdivision_iterations=subdivision_iterations)
    ...
    surf = ax.plot_trisurf(verts[:,0], verts[:,1], verts[:,2], triangles = connectivity)
    plt.savefig(obj_path + 'original_mesh.png')
    plt.close()

    ax =  plt.figure(figsize=(10,10)).add_subplot(projection='3d')
    surf = ax.plot_trisurf(dense_verts[:,0], dense_verts[:,1], dense_verts[:,2], triangles = dense_connectivity)

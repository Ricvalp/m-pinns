...
def densify_mesh(input_obj_file, output_obj_file, subdivision_iterations=1):
    ...
    if not os.path.exists(input_obj_file):
        ...
        try:
            print(f"Loading mesh from: {input_obj_file}")
            ...
            if not mesh.is_watertight:
                # Non-watertight is expected for a 2D surface embedded in 3D
                print("Info: Mesh is not watertight (as expected for a 2D surface).")
            if not len(mesh.faces[0]) == 3:
                raise ValueError("Mesh is not composed of triangles!")

        except Exception as e:
            print(f"Error loading mesh: {e}")
            mesh = None

        # --- Subdivide the Mesh Iteratively ---
        if mesh:
            print(f"\nStarting subdivision ({subdivision_iterations} iterations)...")
            ...
            # --- Export the Dense Mesh ---
            try:
                print(f"\nExporting dense mesh to: {output_obj_file}")
                # Exporting preserves the vertex and face structure
...
#             - new_connectivity is an array of triangles with approximately 4^levels * M triangles
#     """
...
#     """Helper function that performs a single level of mesh densification."""
